...
if __name__ == '__main__':
    ...
    try:
        # 切換檔案主路徑
        main_path = r'D:\Python_Summarize\Python_Training\PERSOL_TAIWAN_OpenCV\Question_1'
        ...
        # 音樂清單
        music_folder = os.path.join(main_path, 'PlayMusic\Japanese_N3')
        music_list = os.listdir(music_folder)
        
        window_title = os.path.split(main_path)[-2] # 視窗標提
        ...
        while camera_control.isOpened():
            ...
            if abs(np.linalg.norm(img - array_comparision_red)) <= 10000 : 
                if not play_music:
                    ...
                    os.system(os.path.join(music_folder, music_list[random.randint(0, len(music_list)-1)]))
                    play_music = True
