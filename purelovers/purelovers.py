            ...
            for i in range(0, len(Excel_todo_list)):
                ...
                try:
                    ...
                    for number in range(1, int(photo_number)+1):
                        ...
                        if download_link == download_link_previous: continue # 檢查連結照片是否已下載，避免連續下載五張No Photo
                        ...
                        download_link_previous = download_link
                        download_link, Img_Save_Path = '',''   
                    # 檢查全部照片是否皆以下載
                    if not already_download == int(photo_number): 
                        Checker_Notes = Checker_Notes + 'Download Error'
                        raise Exception('Download Error')                    
